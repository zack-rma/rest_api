...
public final class RoutingResources extends OpenDcsResource
{
	...
	@Operation(
			...
			description = "The POST routing method requires a valid token. " +
					"It takes a single DECODES Routing Spec in JSON format. " +
	...
	@Operation(
			...
			description = "The POST schedule method requires a valid token. It takes a single DECODES Schedule Entry " +
...
//							examples = {
//									@ExampleObject(ref = "#/components/examples/POST_BASIC_Schedule"),
//									@ExampleObject(ref = "#/components/examples/POST_NEW_Schedule"),
//									@ExampleObject(ref = "#/components/examples/POST_UPDATE_Schedule")
//							})
					)
			),
			responses = {
	...
	@Operation(
			...
			description = "Sample URL:\n  \n    " +
					...
					"The URL must contain a valid user token (either on URL or in HTTP header). " +
	...
	public Response getDacqEvents(
			@Parameter(description = "Only return events generated by a specific app.", example = "26",
					schema = @Schema(implementation = Long.class))
